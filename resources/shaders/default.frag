...
layout(location = 1) out vec4 fragAmbientColor;
layout(location = 2) out vec3 posOutVS;
layout(location = 3) out vec3 normalOutVS;
...
void main() {
	...
	fragColor.a = 1;
	posOutVS = vec3(posVS);
	normalOutVS = vec3(normalize(normalVS));
}
